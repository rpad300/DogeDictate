...
if __name__ == "__main__":
    ...
    try:
        import subprocess
        icns_path = os.path.join(script_dir, "app_icon.icns")
        if os.name != 'nt':  # Not Windows
            # Create temporary directory
            temp_dir = os.path.join(script_dir, "temp_iconset")
            ...
            for size in sizes:
                ...
                if size <= 512:  # Also generate 2x versions for Retina
                    img = Image.open(png_path)
                    img = img.resize((size*2, size*2), Image.LANCZOS)
                    img.save(os.path.join(temp_dir, f"icon_{size}x{size}@2x.png"))
            ...
            print(f"ICNS file generated and saved to: {icns_path}")
    except Exception as e:
        print(f"Failed to generate ICNS file: {str(e)}") 